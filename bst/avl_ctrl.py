...
from bst.avl_model import AVLModel
from bst.avl_view import AVLViewWithPersistence
...
class AVLController(QWidget):
    ...
    def __init__(self, global_ctrl: GlobalController):
        super().__init__()
        self.model = AVLModel()
        self.view = AVLViewWithPersistence(global_ctrl)
    ...
    def _save_to_file(self):
        ...
        if not snapshot["nodes"]:
            QMessageBox.information(self, "AVL", "当前树为空，无需保存。")
            return

        base_dir = Path(__file__).resolve().parents[1] / "save_file" / "avl"
        base_dir.mkdir(parents=True, exist_ok=True)
        suggested = str(base_dir / "avl.json")
        ...
        QMessageBox.information(self, "AVL", f"已保存到：\n{path}")
    ...
    def _parse_sequence(text: str):
        ...
        if not tokens:
            return []
        return [AVLController._coerce_value(tok) for tok in tokens]
